...
public class Debug {
    ...
    public static void logWarning(String message) {
        ...
        if (altoClef != null && !altoClef.getModSettings().shouldHideAllWarningLogs()) {
            if (MinecraftClient.getInstance() != null && MinecraftClient.getInstance().player != null) {
                String msg = getLogPrefix() + "\u00A7c" + message + "\u00A7r";
    ...
    public static void logError(String message) {
        ...
        if (MinecraftClient.getInstance() != null && MinecraftClient.getInstance().player != null) {
            String msg = getLogPrefix() + "[ERROR] " + message + "\nat:\n" + stacktrace + "\u00A7r";
            MinecraftClient.getInstance().player.sendMessage(Text.of(msg), false);
        }
    }
    ...
    private static boolean canLog(int level) {
        if (Rinforzando.getInstance() == null || Rinforzando.getInstance().getModSettings() == null) return true;

        String enabledLogLevel = Rinforzando.getInstance().getModSettings().getLogLevel();
