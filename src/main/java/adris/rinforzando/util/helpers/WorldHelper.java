...
import adris.rinforzando.multiversion.MethodWrapper;
import adris.rinforzando.multiversion.world.WorldVer;
...
public interface WorldHelper {
    ...
    static boolean isSourceBlock(BlockPos pos, boolean onlyAcceptStill) {
        ClientWorld world = Rinforzando.getInstance().getWorld();
    ...
    static BlockPos getChestLeft(BlockPos posWithChest) {
        BlockState state = Rinforzando.getInstance().getWorld().getBlockState(posWithChest);
    ...
    static BlockPos getADesertTemple() {
        ...
        List<BlockPos> stonePressurePlates = Rinforzando.getInstance().getBlockScanner().getKnownLocations(Blocks.STONE_PRESSURE_PLATE);
    ...
    static boolean isUnopenedChest(BlockPos pos) {
        return Rinforzando.getInstance().getItemStorage().getContainerAtPosition(pos).isEmpty();
    }
    ...
    static boolean canBreak(BlockPos pos) {
        Rinforzando altoClef = Rinforzando.getInstance();
    ...
    static boolean isInNetherPortal() {
        ...
        if (player == null)
            return false;
        return adris.rinforzando.multiversion.entity.EntityHelper.isInNetherPortal(player);
    }
    ...
    static boolean canPlace(BlockPos pos) {
        return !Rinforzando.getInstance().getExtraBaritoneSettings().shouldAvoidPlacingAt(pos)
                && canReach(pos);
    }
    ...
    static boolean isAir(BlockPos pos) {
        return Rinforzando.getInstance().getBlockScanner().isBlockAtPosition(pos, Blocks.AIR, Blocks.CAVE_AIR, Blocks.VOID_AIR);
    ...
    static boolean isInteractableBlock(BlockPos pos) {
        Block block = Rinforzando.getInstance().getWorld().getBlockState(pos).getBlock();
    ...
    static boolean isInsidePlayer(BlockPos pos) {
        return pos.isWithinDistance(Rinforzando.getInstance().getPlayer().getPos(), 2);
    }

    static Iterable<BlockPos> getBlocksTouchingPlayer() {
        return getBlocksTouchingBox(Rinforzando.getInstance().getPlayer().getBoundingBox());
    }
    ...
    static boolean isChest(BlockPos block) {
        Block b = Rinforzando.getInstance().getWorld().getBlockState(block).getBlock();
        return isChest(b);
    }
    ...
    static boolean isBlock(BlockPos pos, Block block) {
        return Rinforzando.getInstance().getWorld().getBlockState(pos).getBlock() == block;
    }
    ...
    static boolean isSurroundedByHostiles() {
        List<LivingEntity> hostiles = Rinforzando.getInstance().getEntityTracker().getHostiles();
