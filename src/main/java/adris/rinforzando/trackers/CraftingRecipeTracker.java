...
import adris.rinforzando.multiversion.recipemanager.RecipeManagerWrapper;
import adris.rinforzando.multiversion.recipemanager.WrappedRecipeEntry;
...
public class CraftingRecipeTracker extends Tracker{
    ...
    private final HashMap<Item, List<adris.rinforzando.util.CraftingRecipe>> itemRecipeMap = new HashMap<>();
    private final HashMap<adris.rinforzando.util.CraftingRecipe, ItemStack> recipeResultMap = new HashMap<>();
    ...
    public List<adris.rinforzando.util.CraftingRecipe> getRecipeForItem(Item item) {
        ensureUpdated();
    ...
    public adris.rinforzando.util.CraftingRecipe getFirstRecipeForItem(Item item) {
        ensureUpdated();
    ...
    public List<RecipeTarget> getRecipeTarget(Item item, int targetCount) {
        ...
        for (adris.rinforzando.util.CraftingRecipe recipe : getRecipeForItem(item)) {
    ...
    public ItemStack getRecipeResult(adris.rinforzando.util.CraftingRecipe recipe) {
        ensureUpdated();
    ...
    public boolean hasRecipe(adris.rinforzando.util.CraftingRecipe recipe) {
        ensureUpdated();
    ...
    protected void updateState() {
        ...
        for (WrappedRecipeEntry recipe : recipeManager.values()) {
            ...
            Item[][] rinforzandoRecipeItems = getShapedCraftingRecipe(craftingRecipe.getIngredients());

            adris.rinforzando.util.CraftingRecipe rinforzandoRecipe = adris.rinforzando.util.CraftingRecipe.newShapedRecipe(rinforzandoRecipeItems, result.getCount());

            if (itemRecipeMap.containsKey(result.getItem())) {
                itemRecipeMap.get(result.getItem()).add(rinforzandoRecipe);
            } else {
                List<adris.rinforzando.util.CraftingRecipe> recipes = new ArrayList<>();
                recipes.add(rinforzandoRecipe);

                itemRecipeMap.put(result.getItem(), recipes);
            }

            recipeResultMap.put(rinforzandoRecipe, result);
        }
