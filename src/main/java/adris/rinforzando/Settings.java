...
import adris.rinforzando.util.serialization.IFailableConfigFile;
...
public class Settings implements IFailableConfigFile {
    ...
    public static final String SETTINGS_PATH = "rinforzando_settings.json";
    ...
    /**
     * If true, shows a debug bar displaying how much milliseconds is spend on rinforzando's tick function
     */
     ...
     * <p>
     * This is mainly here because rinforzando does NOT know how to deal with oceans
     */
     ...
     * When running rinforzando commands, if the prefix is not !, the set prefix should be used for the command and not !
    ...
    public Item[] getThrowawayItems(boolean includeProtected) {
        return throwawayItems.stream().filter(item -> includeProtected || !Rinforzando.getInstance().getBehaviour().isProtected(item)).toArray(Item[]::new);
    }

    public Item[] getThrowawayItems(Rinforzando mod) {
