...
import adris.rinforzando.util.time.Stopwatch;
...
public class UserTaskChain extends SingleTaskChain {
    ...
    public void cancel(Rinforzando mod) {
    ...
    public void runTask(Rinforzando mod, Task task, Runnable onFinish) {
    ...
    protected void onTaskFinish(Rinforzando mod) {
        ...
        if (actuallyDone) {
            ...
            if (shouldIdle) {
                Rinforzando.getCommandExecutor().executeWithPrefix(mod.getModSettings().getIdleCommand());
