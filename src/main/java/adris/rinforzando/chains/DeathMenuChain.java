...
import adris.rinforzando.mixins.DeathScreenAccessor;
import adris.rinforzando.multiversion.ConnectScreenVer;
...
import adris.rinforzando.util.time.TimerReal;
...
public class DeathMenuChain extends TaskChain {
    ...
    private boolean shouldAutoRespawn() {
        return Rinforzando.getInstance().getModSettings().isAutoRespawn();
    }

    private boolean shouldAutoReconnect() {
        return Rinforzando.getInstance().getModSettings().isAutoReconnect();
    }
        ...
        if (screen instanceof DeathScreen) {
                    ...
                    for (String i : mod.getModSettings().getDeathCommand().split(" & ")) {
                        ...
                        if (!command.isEmpty()) {
                            if (command.startsWith(prefix)) {
                                Rinforzando.getCommandExecutor().execute(command, () -> {
            ...
            if (screen instanceof DisconnectedScreen) {
                if (shouldAutoReconnect()) {
                    ...
                    Rinforzando.getInstance().cancelUserTask();
                }
