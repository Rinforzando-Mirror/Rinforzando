...
import adris.rinforzando.control.KillAura;
import adris.rinforzando.multiversion.versionedfields.Entities;
import adris.rinforzando.multiversion.item.ItemVer;
import adris.rinforzando.tasks.construction.ProjectileProtectionWallTask;
import adris.rinforzando.tasks.entity.KillEntitiesTask;
import adris.rinforzando.tasks.movement.CustomBaritoneGoalTask;
import adris.rinforzando.tasks.movement.DodgeProjectilesTask;
import adris.rinforzando.tasks.movement.RunAwayFromCreepersTask;
import adris.rinforzando.tasks.movement.RunAwayFromHostilesTask;
import adris.rinforzando.tasks.speedrun.DragonBreathTracker;
import adris.rinforzando.tasksystem.TaskRunner;
import adris.rinforzando.util.baritone.CachedProjectile;
...
public class MobDefenseChain extends SingleTaskChain {
    ...
    private static void startShielding(Rinforzando mod) {
        shielding = true;
    ...
    public float getPriority() {
        ...
        prevHealth = Rinforzando.getInstance().getPlayer().getHealth();
        return cachedLastPriority;
    }

    private void stopShielding(Rinforzando mod) {
        if (shielding) {
    ...
    private boolean escapeDragonBreath(Rinforzando mod) {
    ...
    private static boolean hasShield(Rinforzando mod) {
        return mod.getItemStorage().hasItem(Items.SHIELD) || mod.getItemStorage().hasItemInOffhand(Items.SHIELD);
    }

    private static SwordItem getBestSword(Rinforzando mod) {
    ...
    private BlockPos isInsideFireAndOnFire(Rinforzando mod) {
    ...
    private void putOutFire(Rinforzando mod, BlockPos pos) {
    ...
    private void doForceField(Rinforzando mod) {
        killAura.tickStart();
    ...
    private CreeperEntity getClosestFusingCreeper(Rinforzando mod) {
    ...
    private boolean isProjectileClose(Rinforzando mod) {
    ...
    private Optional<Entity> getUniversallyDangerousMob(Rinforzando mod) {
    ...
    private boolean isInDanger(Rinforzando mod) {
    ...
    @Override
    protected void onTaskFinish(Rinforzando mod) {
