...
import adris.rinforzando.tasks.construction.PutOutFireTask;
import adris.rinforzando.tasks.movement.EnterNetherPortalTask;
import adris.rinforzando.tasks.movement.EscapeFromLavaTask;
import adris.rinforzando.tasks.movement.GetToBlockTask;
import adris.rinforzando.tasks.movement.SafeRandomShimmyTask;
...
public class WorldSurvivalChain extends SingleTaskChain {
    ...
    public float getPriority() {
        if (!Rinforzando.inGame()) return Float.NEGATIVE_INFINITY;
    ...
    private void handleDrowning(Rinforzando mod) {
        // Swim
    ...
    private boolean isInLavaOhShit(Rinforzando mod) {
    ...
    private boolean isInFire(Rinforzando mod) {
    ...
    private boolean isStuckInNetherPortal() {
        return WorldHelper.isInNetherPortal()
                && !Rinforzando.getInstance().getUserTaskChain().getCurrentTask().thisOrChildSatisfies(task -> task instanceof EnterNetherPortalTask);
    }
