...
public abstract class AbstractDoToClosestObjectTask<T> extends Task {
    ...
    protected abstract Vec3d getPos(Rinforzando mod, T obj);

    protected abstract Optional<T> getClosestTo(Rinforzando mod, Vec3d pos);

    protected abstract Vec3d getOriginPos(Rinforzando mod);

    protected abstract Task getGoalTask(T obj);

    protected abstract boolean isValid(Rinforzando mod, T obj);

    // Virtual
    protected Task getWanderTask(Rinforzando mod) {
    ...
    private double getCurrentCalculatedHeuristic(Rinforzando mod) {
