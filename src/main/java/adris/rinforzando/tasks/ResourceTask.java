...
import adris.rinforzando.tasks.container.PickupFromContainerTask;
...
import adris.rinforzando.tasks.resources.MineAndCollectTask;
import adris.rinforzando.tasks.slot.EnsureFreePlayerCraftingGridTask;
import adris.rinforzando.tasks.slot.MoveInaccessibleItemToInventoryTask;
import adris.rinforzando.tasksystem.ITaskCanForce;
import adris.rinforzando.tasksystem.ITaskUsesCraftingGrid;
...
import adris.rinforzando.util.helpers.StlHelper;
...
public abstract class ResourceTask extends Task implements ITaskCanForce {
    ...
    protected void onStart() {
        ...
        onResourceStart(Rinforzando.getInstance());
    }
    ...
    protected void onStop(Task interruptTask) {
        ...
        onResourceStop(Rinforzando.getInstance(), interruptTask);
    }
    ...
    protected boolean isInWrongDimension(Rinforzando mod) {
        if (forceDimension) {
    ...
    protected Task getToCorrectDimensionTask(Rinforzando mod) {
    ...
    protected abstract boolean shouldAvoidPickingUp(Rinforzando mod);

    protected abstract void onResourceStart(Rinforzando mod);

    protected abstract Task onResourceTick(Rinforzando mod);

    protected abstract void onResourceStop(Rinforzando mod, Task interruptTask);
