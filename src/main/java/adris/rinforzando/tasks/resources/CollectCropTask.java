...
public class CollectCropTask extends ResourceTask {
    ...
    public boolean isFinished() {
        ...
        if (shouldReplantNow(Rinforzando.getInstance())) {
            return false;
        }
        return super.isFinished();
    }

    private boolean shouldReplantNow(Rinforzando mod) {
        return mod.getModSettings().shouldReplantCrops() && hasEmptyCrops(mod) && mod.getItemStorage().hasItem(_cropSeed);
    }

    private boolean hasEmptyCrops(Rinforzando mod) {
    ...
    private boolean isEmptyCrop(Rinforzando mod, BlockPos pos) {
    ...
    private boolean isMature(Rinforzando mod, BlockPos blockPos) {
