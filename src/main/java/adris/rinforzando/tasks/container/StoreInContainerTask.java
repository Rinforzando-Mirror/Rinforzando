...
import adris.rinforzando.tasks.slot.MoveItemToSlotFromInventoryTask;
...
public class StoreInContainerTask extends AbstractDoToStorageContainerTask {
    ...
    protected void onStart() {
        ...
        if (storedItems == null) {
            ...
            storedItems = new ContainerStoredTracker(slot -> {
                Optional<BlockPos> openContainer = Rinforzando.getInstance().getItemStorage().getLastBlockPosInteraction();
    ...
    protected Task onTick() {
        ...
        if (getIfNotPresent) {
            for (ItemTarget target : toStore) {
                ...
                if (inventoryNeed > Rinforzando.getInstance().getItemStorage().getItemCount(target)) {
    ...
    public boolean isFinished() {
        ...
        return storedItems != null && storedItems.getUnstoredItemTargetsYouCanStore(Rinforzando.getInstance(), toStore).length == 0;
    }
