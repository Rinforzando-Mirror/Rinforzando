...
import adris.rinforzando.tasks.slot.EnsureFreeInventorySlotTask;
...
import adris.rinforzando.util.slots.FurnaceSlot;
...
public class PickupFromContainerTask extends AbstractDoToStorageContainerTask {
    ...
    public static Optional<Slot> getBestSlotToTransfer(Rinforzando mod, ItemTarget itemToMove, int currentItemQuantity, List<Slot> grabPotentials, Function<ItemStack, Boolean> canStackFit) {
    ...
    protected Task onTick() {
        ...
        if (_freeInventoryTask.isActive() && !_freeInventoryTask.isFinished() && !Rinforzando.getInstance().getItemStorage().hasEmptyInventorySlot()) {
    ...
    public boolean isFinished() {
        return Arrays.stream(_targets).allMatch(target -> Rinforzando.getInstance().getItemStorage().getItemCountInventoryOnly(target.getMatches()) >= target.getTargetCount());
    }

    @Override
    protected Task onContainerOpenSubtask(Rinforzando mod, ContainerCache containerCache) {
