...
public class ProjectileProtectionWallTask extends Task implements ITaskRequiresGrounded {
	...
	public ProjectileProtectionWallTask(Rinforzando mod) {
		this.mod = mod;
	}
	...
	protected Task onTick() {
		if (targetPlacePos != null && !WorldHelper.isSolidBlock(targetPlacePos)) {
			Optional<adris.rinforzando.util.slots.Slot> slot = StorageHelper.getSlotWithThrowawayBlock(this.mod, true);
			if(slot.isPresent()) {
	...
	public boolean canPlace(BlockPos blockPos, boolean checkEntities) {
        ...
        // Check y level
        if (!World.isValid(blockPos) || !Rinforzando.getInstance().getWorld().isInBuildLimit(blockPos)) return false;
