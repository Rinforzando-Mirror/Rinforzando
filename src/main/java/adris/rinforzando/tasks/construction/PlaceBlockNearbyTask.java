...
import adris.rinforzando.eventbus.Subscription;
import adris.rinforzando.eventbus.events.BlockPlaceEvent;
...
public class PlaceBlockNearbyTask extends Task {
    ...
    protected void onStart() {
        ...
        Rinforzando.getInstance().getClientBaritone().getInputOverrideHandler().setInputForceState(Input.CLICK_RIGHT, false);
    ...
    public boolean isFinished() {
        return justPlaced != null && ArrayUtils.contains(toPlace, Rinforzando.getInstance().getWorld().getBlockState(justPlaced).getBlock());
    }
    ...
    private BlockPos getCurrentlyLookingBlockPlace(Rinforzando mod) {
    ...
    private boolean place(Rinforzando mod, BlockPos targetPlace) {
    ...
    private void stopPlacing() {
        Rinforzando.getInstance().getInputControls().release(Input.SNEAK);
        //mod.getControllerExtras().mouseClickOverride(1, false);
        // Oof, these sometimes cause issues so this is a bit of a duct tape fix.
        Rinforzando.getInstance().getClientBaritone().getBuilderProcess().onLostControl();
    }

    private BlockPos locateClosePlacePos(Rinforzando mod) {
        int range = 7;
        BlockPos best = null;
