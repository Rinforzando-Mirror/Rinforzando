...
import adris.rinforzando.multiversion.DamageSourceVer;
import adris.rinforzando.tasksystem.Task;
import adris.rinforzando.util.helpers.*;
import adris.rinforzando.util.serialization.ItemDeserializer;
import adris.rinforzando.util.serialization.ItemSerializer;
...
public class MLGBucketTask extends Task {
    ...
    private static void moveLeftRight(int delta) {
        InputControls controls = Rinforzando.getInstance().getInputControls();
    ...
    private Task onTickInternal(Rinforzando mod, BlockPos oldMovingTorwards) {
    ...
    private Task placeMLGBucketTask(Rinforzando mod, BlockPos toPlaceOn) {
    ...
    private void handleForwardVelocity(Rinforzando mod, boolean newForwardTarget) {
    ...
    protected void onStart() {
        Rinforzando.getInstance().getClientBaritone().getPathingBehavior().forceCancel();
        placedPos = null;
        // hold shift while falling.
        // Look down at first, might help
        Rinforzando.getInstance().getPlayer().setPitch(90);
    }
     ...
     */
    private void handleJumpForLand(Rinforzando mod, BlockPos willLandOn) {
    ...
    private Optional<BlockPos> getBlockWeWillLandOn(Rinforzando mod) {
     ...
     */
    private void handleCancellingSidewaysVelocity(Rinforzando mod) {
    ...
    private Optional<BlockPos> getBestConeClutchBlock(Rinforzando mod, BlockPos oldClutchTarget) {
    ...
    protected void onStop(Task interruptTask) {
        Baritone baritone = Rinforzando.getInstance().getClientBaritone();
        InputControls controls = Rinforzando.getInstance().getInputControls();
    ...
    private boolean hasClutchItem(Rinforzando mod) {
    ...
    class ConeClutchContext {
        ...
        public ConeClutchContext(Rinforzando mod) {
            hasClutchItem = hasClutchItem(mod);
        }

        public void checkBlock(Rinforzando mod, BlockPos check) {
            // Already checked
        ...
        public void checkRay(Rinforzando mod, RaycastContext rctx) {
            BlockHitResult hit = mod.getWorld().raycast(rctx);
            if (hit.getType() == HitResult.Type.BLOCK) {
