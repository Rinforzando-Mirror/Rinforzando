...
public abstract class SearchChunksExploreTask extends Task {
    ...
    // Virtual
    protected ChunkPos getBestChunkOverride(Rinforzando mod, List<ChunkPos> chunks) {
        return null;
    }
    ...
    private void onChunkLoad(ChunkPos pos) {
        ...
        if (isChunkWithinSearchSpace(Rinforzando.getInstance(), pos)) {
            synchronized (searcherMutex) {
    ...
    protected abstract boolean isChunkWithinSearchSpace(Rinforzando mod, ChunkPos pos);
    ...
    public void resetSearch() {
        ...
        for (ChunkPos start : Rinforzando.getInstance().getChunkTracker().getLoadedChunks()) {
            onChunkLoad(start);
        }
    }
    ...
    class SearchSubTask extends ChunkSearchTask {
        ...
        @Override
        protected boolean isChunkPartOfSearchSpace(Rinforzando mod, ChunkPos pos) {
            return isChunkWithinSearchSpace(mod, pos);
        }

        @Override
        public ChunkPos getBestChunk(Rinforzando mod, List<ChunkPos> chunks) {
