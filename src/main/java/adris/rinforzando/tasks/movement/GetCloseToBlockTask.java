...
public class GetCloseToBlockTask extends Task {
    ...
    private int getCurrentDistance() {
        return (int) Math.sqrt(Rinforzando.getInstance().getPlayer().getBlockPos().getSquaredDistance(_toApproach));
    }

    private boolean inRange() {
        return Rinforzando.getInstance().getPlayer().getBlockPos().getSquaredDistance(_toApproach) <= _currentRange * _currentRange;
    }
