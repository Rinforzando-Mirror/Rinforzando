...
abstract class ChunkSearchTask extends Task {
    ...
    protected void onStart() {
        ...
        if (_first) {
            _finished = false;
            _first = false;
            ChunkPos startPos = Rinforzando.getInstance().getWorld().getChunk(_startPoint).getPos();
            synchronized (_searchMutex) {
                searchChunkOrQueueSearch(Rinforzando.getInstance(), startPos);
            }
        }
    ...
    protected Task onTick() {
            ...
            if (!_justLoaded.isEmpty()) {
                for (ChunkPos justLoaded : _justLoaded) {
                    if (_searchLater.contains(justLoaded)) {
                        ...
                        if (trySearchChunk(Rinforzando.getInstance(), justLoaded)) {
        ...
        ChunkPos closest = getBestChunk(Rinforzando.getInstance(), _searchLater);
    ...
    // Virtual
    protected ChunkPos getBestChunk(Rinforzando mod, List<ChunkPos> chunks) {
    ...
    private void searchChunkOrQueueSearch(Rinforzando mod, ChunkPos pos) {
     ...
     */
    private boolean trySearchChunk(Rinforzando mod, ChunkPos pos) {
        // Do NOT search later.
        if (_searchedAlready.contains(pos)) {
            return true;
        }
    ...
    protected abstract boolean isChunkPartOfSearchSpace(Rinforzando mod, ChunkPos pos);
