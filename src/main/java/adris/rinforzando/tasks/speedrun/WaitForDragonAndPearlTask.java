...
import adris.rinforzando.tasks.movement.ThrowEnderPearlSimpleProjectileTask;
import adris.rinforzando.tasks.resources.GetBuildingMaterialsTask;
...
public class WaitForDragonAndPearlTask extends Task {
    ...
    private boolean canThrowPearl(Rinforzando mod) {
    ...
    private boolean isFireballDangerous(Rinforzando mod, Optional<Entity> fireball) {
        if (fireball.isEmpty())
            return false;
    ...
    public boolean isFinished() {
        return dragonIsPerching
                ...
                || WorldHelper.inRangeXZ(Rinforzando.getInstance().getPlayer(), targetToPearl, CLOSE_ENOUGH_DISTANCE));
    }
