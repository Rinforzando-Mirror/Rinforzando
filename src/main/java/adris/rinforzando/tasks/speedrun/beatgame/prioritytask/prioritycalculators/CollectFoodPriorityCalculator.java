package adris.rinforzando.tasks.speedrun.beatgame.prioritytask.prioritycalculators;
...
import static adris.rinforzando.tasks.resources.CollectFoodTask.*;
...
public class CollectFoodPriorityCalculator extends ItemPriorityCalculator {
    ...
    public CollectFoodPriorityCalculator(Rinforzando mod ,double foodUnits) {
    ...
    private double getDistance(Rinforzando mod) {
    ...
    private double pickupBlockTaskOrNull(Rinforzando mod, Block blockToCheck, Item itemToGrab, double maxRange) {
        return pickupBlockTaskOrNull(mod, blockToCheck, itemToGrab, toAccept -> true, maxRange);
    }

    private double pickupBlockTaskOrNull(Rinforzando mod, Block blockToCheck, Item itemToGrab, Predicate<BlockPos> accept, double maxRange) {
    ...
    private double pickupTaskOrNull(Rinforzando mod, Item itemToGrab) {
        return pickupTaskOrNull(mod, itemToGrab, Double.POSITIVE_INFINITY);
    }

    private double pickupTaskOrNull(Rinforzando mod, Item itemToGrab, double maxRange) {
        Optional<ItemEntity> nearestDrop = Optional.empty();
        if (mod.getEntityTracker().itemDropped(itemToGrab)) {
            nearestDrop = mod.getEntityTracker().getClosestItemDrop(mod.getPlayer().getPos(), itemToGrab);
        }
