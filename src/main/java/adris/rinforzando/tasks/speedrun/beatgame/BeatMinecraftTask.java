package adris.rinforzando.tasks.speedrun.beatgame;
...
import adris.rinforzando.multiversion.blockpos.BlockPosVer;
import adris.rinforzando.multiversion.versionedfields.Blocks;
import adris.rinforzando.tasks.*;
...
import adris.rinforzando.tasks.construction.PlaceObsidianBucketTask;
import adris.rinforzando.tasks.container.*;
import adris.rinforzando.tasks.misc.EquipArmorTask;
import adris.rinforzando.tasks.misc.PlaceBedAndSetSpawnTask;
import adris.rinforzando.tasks.misc.SleepThroughNightTask;
import adris.rinforzando.tasks.movement.*;
import adris.rinforzando.tasks.resources.*;
import adris.rinforzando.tasks.speedrun.*;
import adris.rinforzando.tasks.speedrun.beatgame.prioritytask.prioritycalculators.CollectFoodPriorityCalculator;
import adris.rinforzando.tasks.speedrun.beatgame.prioritytask.prioritycalculators.DistanceItemPriorityCalculator;
import adris.rinforzando.tasks.speedrun.beatgame.prioritytask.prioritycalculators.StaticItemPriorityCalculator;
import adris.rinforzando.tasks.speedrun.beatgame.prioritytask.tasks.*;
import adris.rinforzando.tasksystem.Task;
import adris.rinforzando.trackers.EntityTracker;
import adris.rinforzando.trackers.storage.ItemStorageTracker;
import adris.rinforzando.util.*;
import adris.rinforzando.util.helpers.*;
...
//#if MC <= 12006
//$$ import adris.rinforzando.mixins.EntityAccessor;
//#endif
...
import static adris.rinforzando.tasks.resources.CollectMeatTask.COOKABLE_FOODS;
...
public class BeatMinecraftTask extends Task {
    ...
    public BeatMinecraftTask(Rinforzando mod) {
        this.mod = mod;
     ...
     */
    private static List<BlockPos> getFrameBlocks(Rinforzando mod,BlockPos endPortalCenter) {
     ...
     *
     * @param mod The Rinforzando mod instance.
     * @param pos The position of the End Portal Frame.
     * @return True if the End Portal Frame is filled, false otherwise.
     */
    private static boolean isEndPortalFrameFilled(Rinforzando mod, BlockPos pos) {
     ...
     *
     * @param mod  The Rinforzando mod.
     * @param task The task to check.
     * @return True if the task is running, false otherwise.
     */
    public static boolean isTaskRunning(Rinforzando mod, Task task) {
        if (task == null) {
    ...
    public static void throwAwayItems(Rinforzando mod, Item... items) {
        throwAwaySlots(mod, mod.getItemStorage().getSlotsWithItemPlayerInventory(false, items));
    }

    public static void throwAwaySlots(Rinforzando mod, List<Slot> slots) {
        for (Slot slot : slots) {
    ...
    public static boolean hasItem(Rinforzando mod, Item item) {
    ...
    public static int getCountWithCraftedFromOre(Rinforzando mod, Item item) {
        ItemStorageTracker itemStorage = mod.getItemStorage();
    ...
    private void addSleepTask(Rinforzando mod) {
    ...
    private void addPickupImportantItemsTask(Rinforzando mod) {
    ...
    private void addCookFoodTasks(Rinforzando mod) {
        gatherResources.add(new ActionPriorityTask(a -> {
    ...
    private void addSmeltTasks(Rinforzando mod) {
        ItemStorageTracker itemStorage = mod.getItemStorage();
    ...
    private void addLootChestsTasks(Rinforzando mod) {
    ...
    private void addCollectFoodTask(Rinforzando mod) {
    ...
    private void addDiamondArmorTasks(Rinforzando mod) {
    ...
    private void addPickaxeTasks(Rinforzando mod) {
     ...
     */
    private boolean needsBuildingMaterials(Rinforzando mod) {
     ...
     * @param mod The Rinforzando mod instance.
     */
    private void updateCachedEndItems(Rinforzando mod) {
     ...
     */
    private List<Item> lootableItems(Rinforzando mod) {
     ...
     *
     * @param mod             The instance of the Rinforzando mod.
     * @param endPortalCenter The center position of the end portal.
     * @return True if the end portal has been found, false otherwise.
     */
    private boolean endPortalFound(Rinforzando mod, BlockPos endPortalCenter) {
     ...
     */
    private boolean endPortalOpened(Rinforzando mod, BlockPos endPortalCenter) {
     ...
     */
    private boolean spawnSetNearPortal(Rinforzando mod, BlockPos endPortalCenter) {
     ...
     */
    private Optional<BlockPos> locateClosestUnopenedChest(Rinforzando mod) {
     ...
     * @param mod The Rinforzando mod instance.
     */
    private void addThrowawayItemsWarning(Rinforzando mod) {
        // Warning message tail that will be appended to the warning message.
        String settingsWarningTail = "in \".minecraft/rinforzando_settings.json\". !gamer may break if you don't add this! (sorry!)";
    ...
    /**
     * Adds protected items to the behaviour of the given Rinforzando instance.
     *
     * @param mod The Rinforzando instance.
     */
    private void addProtectedItems(Rinforzando mod) {
     ...
     * @param mod The Rinforzando mod instance.
     */
    private void allowWalkingOnEndPortal(Rinforzando mod) {
     ...
     * @param mod The Rinforzando mod instance.
     */
    private void avoidDragonBreath(Rinforzando mod) {
     ...
     * @param mod The Rinforzando mod instance.
     */
    private void avoidBreakingBed(Rinforzando mod) {
    ...
    private void blackListDangerousBlock(Rinforzando mod, Block block) {
     ...
     */
    private Task setSpawnNearPortalTask(Rinforzando mod) {
     ...
     */
    private Task getBlazeRodsTask(Rinforzando mod, int count) {
     ...
     */
    private Task getEnderPearlTask(Rinforzando mod, int count) {
     ...
     */
    private int getTargetBeds(Rinforzando mod) {
     ...
     */
    private boolean needsBeds(Rinforzando mod) {
     ...
     */
    private Task getBedTask(Rinforzando mod) {
        int targetBeds = getTargetBeds(mod);
     ...
     */
    private boolean anyBedsFound(Rinforzando mod) {
    ...
    private BlockPos doSimpleSearchForEndPortal(Rinforzando mod) {
     ...
     */
    private int getFilledPortalFrames(Rinforzando mod, BlockPos endPortalCenter) {
        if (endPortalCenter == null) {
            return 0;
        }
     ...
     */
    private boolean canBeLootablePortalChest(Rinforzando mod, BlockPos blockPos) {
    ...
    private Task getEyesOfEnderTask(Rinforzando mod, int targetEyes) {
