...
import adris.rinforzando.commandsystem.CommandExecutor;
import adris.rinforzando.commandsystem.TabCompleter;
import adris.rinforzando.control.InputControls;
import adris.rinforzando.control.PlayerExtraController;
import adris.rinforzando.control.SlotHandler;
import adris.rinforzando.eventbus.EventBus;
import adris.rinforzando.eventbus.events.ClientRenderEvent;
import adris.rinforzando.eventbus.events.ClientTickEvent;
import adris.rinforzando.eventbus.events.SendChatEvent;
import adris.rinforzando.eventbus.events.TitleScreenEntryEvent;
import adris.rinforzando.multiversion.DrawContextWrapper;
import adris.rinforzando.multiversion.RenderLayerVer;
...
import adris.rinforzando.trackers.*;
import adris.rinforzando.trackers.storage.ContainerSubTracker;
import adris.rinforzando.trackers.storage.ItemStorageTracker;
import adris.rinforzando.ui.RinforzandoTickChart;
import adris.rinforzando.ui.CommandStatusOverlay;
...
/**
 * Central access point for Rinforzando
 */
public class Rinforzando implements ModInitializer {

    // Static access to rinforzando
    private static final Queue<Consumer<Rinforzando>> _postInitQueue = new ArrayDeque<>();
    ...
    private RinforzandoTickChart tickChart;
    // Settings
    private adris.rinforzando.Settings settings;
    // Misc managers/input
    ...
    private static Rinforzando instance;
...
/** Executes commands (ex. `!get`/`!gamer`) */
    ...
    public void onInitialize() {
        ...
        if (instance != null) {
            throw new IllegalStateException("Rinforzando already loaded!");
        }
        instance = this;
    }
    ...
    public void onInitializeLoad() {
        ...
        tickChart = new RinforzandoTickChart(MinecraftClient.getInstance().textRenderer);
        ...
        // Load settings
        adris.rinforzando.Settings.load(newSettings -> {
...
// Don't break blocks or place blocks where we are explicitly protected.
        ...
        EventBus.subscribe(ClientTickEvent.class, evt -> {
            ...
            onClientTick();
            tickChart.pushTickNanos(System.nanoTime()-nanos);
        });
    ...
    private void onClientRenderOverlay(DrawContextWrapper context) {
        ...
        if (settings.shouldShowDebugTickMs()) {
            tickChart.render(this, context, 1, context.getScaledWindowWidth() / 2 - 124);
        }
    }
     ...
     */
    public static Rinforzando getInstance() {
        return instance;
    }
    ...
    /**
     * Baritone settings special to Rinforzando (could just be static honestly)
     */
    ...
    /**
     * Rinforzando Settings
     */
    public adris.rinforzando.Settings getModSettings() {
        return settings;
    }
