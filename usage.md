...
Rinforzando has a variety of commands, settings and modes. This will give users an overview on how to use the bot.
...
After running the game with the mod once, a folder called `rinforzando` should appear in your `.minecraft` directory. This
contains `rinforzando` related settings and configurations.

With regards to the `rinforzando_settings.json` file
Check [Settings.java](https://github.com/gaucho-matrero/rinforzando/blob/main/src/main/java/adris/rinforzando/Settings.java)
...
You'll have to append each player's name to a file called `rinforzando_butler_whitelist.txt`
located in your `.minecraft/rinforzando` directory. Make sure your name is not found in `rinforzando_butler_blacklist.txt`
too.
...
being
picked up by rinforzando, you can append the server's custom whisper format in the `whisperFormats` setting.
